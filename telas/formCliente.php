...
<body>
	...
	<div class="container bg-secondary" style="--bs-bg-opacity: .25;">
		...
		<div class="row">
			...
			<div class="col-lg-4 ">
				<form id="form_pesquisa_titulo" action="index.php?mod=cliente" method="post" >
			...
			<div class="col-lg-8 ps-4">
				<form action="index.php?mod=cliente" method="post" onsubmit="return validar();">
					<input type="hidden" id="hdnId" name="hdnId" value="<?= isset($_cliente->id)?$_cliente->id:0 ?>"/>					
					
					<div class="row">
						<div class="col-12">
							<label for="txtTitulo" class="form-label">Nome: </label>
							<input class="form-control " id="txtNome" name="txtNome" value="<?= isset($_cliente->nome)?$_cliente->nome:'' ?>" >
						</div>
					</div>
					<div class="row">
						<div class="col-12">
							<label for="txtTitulo" class="form-label">Endereço:</label>
							<input class="form-control " id="txtLogradouro" name="txtLogradouro" value="<?= isset($_cliente->logradouro)?$_cliente->logradouro:'' ?>" >
						</div>
					</div>
					<div class="row">
						<div class="col-12">
							<label for="txtDescricao" class="form-label">Bairro:</label>
							<input class="form-control " id="txtBairro" name="txtBairro" value="<?= isset($_cliente->bairro)?$_cliente->bairro:'' ?>" >							
						</div>	
					</div>
						<div class="row">
						<div class="col-6">
							<label for="txtCodigo" class="form-label">CPF::</label>		
							<input class="form-control " id="txtCPF" name="txtCPF" value="<?= isset($_cliente->cpf)?$_cliente->cpf:'' ?>" >				
							
						</div>
						<div class="col-6 ">
							<label for="txtQdeEstoque" class="form-label">RG:</label>
							<input class="form-control " id="txtRG" name="txtRG" value="<?= isset($_cliente->rg)?$_cliente->rg:'' ?>" >						
					...
					<div class="row ">
						<div class="input-group-sm text-center pt-4" >
							...
							<a href="index.php?mod=cliente" class="btn btn-warning ms-2">Cancelar</a>
				...
				<script>
					<?= empty($_POST)||(!isset($_POST['btnNovaMidia'])&&!isset($_POST['selCliente']))?"inicia_cliente();":"" ?>
