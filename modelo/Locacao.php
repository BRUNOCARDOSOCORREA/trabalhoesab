<?php
	...
	class Locacao {
		...
		public function __construct($path){		
			...
			$this->cliente = new Cliente($path);
			$this->funcionario = new Funcionario($path);			
		}
		
		public function adicionarItem( $item ){
			$this->listaItemLocacao[$item->codigo] = $item;
		}
		
		public function retirarItem($codigo){
			unset($this->listaItemLocacao[$codigo]);
		}
		
		public function salvarItensLocacao(){
			$sql = "INSERT INTO 'main'.'tb_item_locacao' ('id_locacao', 'id_item_acervo', 'valor') VALUES ";
			foreach($this->listaItemLocacao as $cod=>$item){
				// valor de locacao do item está fixo
				$sql .=  "('{$this->id}', '{$item->id}', '1.00'),";
			}
		...
		public function salvar(){			
			...
			if($this->id==0){		
				...
				$sql = "INSERT INTO 'main'.'tb_locacao' ('id_cliente', 'id_funcionario', 'data_locacao','valor_base',  'valor_total', 'data_entrega') VALUES (:id_cliente, :id_funcionario,date() , :valor_base, :valor_total,:data_entrega );";
			
				$stmt = $this->con->prepare($sql);			
			
				$stmt->bindParam(':id_cliente',$this->cliente->id);
				$stmt->bindParam(':id_funcionario',$this->funcionario->id);
				$stmt->bindParam(':data_entrega',$this->data_entrega);
				$stmt->bindParam(':valor_base',$this->valor_base);		
				$stmt->bindParam(':valor_total',$this->valor_total);			
							
				$stmt->execute();
				$this->id = $this->con->lastInsertId();
				$qde = $stmt->rowCount();
				$this->salvarItensLocacao();
			} 
			else {
		...
		public function buscar($id){
			$sql = "SELECT l.*, c.*, l.id as id_locacao,c.id as id_cliente, f.id as id_funcionario 
						FROM tb_locacao AS l
						INNER JOIN tb_cliente AS c ON (l.id_cliente = c.id)
						INNER JOIN tb_funcionario AS f ON (l.id_funcionario = f.id OR l.id_funcionario=0)
						WHERE 
 						l.id = :cod";
			...
			if ($row != null){ 
				...
				$loc->data_entrega = implode('/', array_reverse(explode('-', $row['data_entrega'])));; ;;
		...
		public function verificarExistenciaLocacaoItem($cod){
			$sql = "SELECT * FROM tb_locacao AS l
						INNER JOIN tb_item_locacao AS il ON (il.id_locacao = l.id)
						INNER JOIN tb_item_acervo AS ia ON (il.id_item_acervo = ia.id)
						WHERE						 
							ia.codigo = :cod";
